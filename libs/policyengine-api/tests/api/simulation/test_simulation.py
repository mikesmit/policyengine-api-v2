...
def client():
    ...
    auth = JWTDecoder(issuer="test_issuer", audience="test_audience", auto_error=True)
    optional_auth = JWTDecoder(issuer="test_issuer", audience="test_audience", auto_error=False)
...
def test_run_simulation(monkeypatch, client):
    ...
    assert output_data == {"simulated": True}, f"Unexpected simulation result: {output_data}"
